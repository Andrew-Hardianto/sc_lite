...
class _LeaveScreenState extends State<LeaveScreen> {
  ...
  TextEditingController notesLeave = TextEditingController();
  PageController pageCtrl = PageController(initialPage: 0);

  int formPage = 0;
  String? leaveType;
  String? leaveTypeFinal;
  DateTime? leaveStartDate;
  DateTime? leaveStartDateFinal;
  DateTime? eligibleLeaveDate;
  DateTime? leaveEndDate;
  DateTime? leaveEndDateFinal;
  int totalLeaveDate = 0;
  ...
  selectedDate(String type) async {
    DatePicker.showDatePicker(
      ...
      minTime: type == 'end' ? leaveStartDate : DateTime(2000),
      maxTime: DateTime(DateTime.now().year, 12, 31),
      onConfirm: (time) {
        if (type == 'start') {
          leaveStartDate = time;
          leaveStart.text = DateFormat('dd MMM yyyy').format(time);
          leaveEndDate = DateTime.now();
          leaveEnd.clear();
        } else {
          leaveEndDate = time;
          leaveEnd.text = DateFormat('dd MMM yyyy').format(time);
        }
      },
    );
  }
  ...
  pageContent1() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'What are you applying for sick for?',
  ...
  pageContent2() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'Who will replace you when you leave?',
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600),
          ),
          Container(
            height: 50,
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Example : Leave for married',
                border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(10.0),
                ),
          ...
          const Text(
            'Please attach documents related to leave',
            style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600),
          ),
          Container(
            margin: const EdgeInsets.symmetric(vertical: 10),
